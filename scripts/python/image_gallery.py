...
top_src_path = Path(__file__).resolve().parent.parent.parent
...
albums = Path(top_src_path, 'assets/images/photography/*')
print(albums)
print([p for p in albums.iterdir() if p.is_dir()])
sys.exit()
for album_path in [p for p in albums.iterdir() if p.is_dir()]:
    print(album_path)
